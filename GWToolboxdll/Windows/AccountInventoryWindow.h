...
class AccountInventoryWindow : public ToolboxWindow {
    inline static constexpr const wchar_t* FREE_SLOTS_INI_FILENAME = L"accountinventory_freeslots.ini";
    ...
    struct CharacterFreeSlots {
        ...
        int max_inventory{};
        int max_equipment{};
        int occupied_inventory{};
        int occupied_equipment{};
    };
    struct SlotCompare {
        bool operator()(const std::shared_ptr<CharacterFreeSlots> l, const std::shared_ptr<CharacterFreeSlots> r) const
        {
            // the freeSlots set breaks on account change, since the meaning of '<' depends on currentAccount
            // but we rebuild freeSlots on map load anyway, so it's ok.
            const auto c = GW::GetCharContext();
            std::wstring currentAccount = c ? c->player_email : L"";
            if (l->account != r->account) {
                // make sure currentAccount is sorted to the top
                if (l->account == currentAccount) return true;
                if (r->account == currentAccount) return false;
            }
            if (l->character != r->character) return l->character < r->character;
    ...
    {
        freeSlotsIni = new ToolboxIni(false, false, false);
        show_menubutton = can_show_in_main_window;
    }

    ~AccountInventoryWindow() override
    {
        delete freeSlotsIni;
    }
...
private:
    ...
    static std::wstring getIniID(std::shared_ptr<InventoryItem> i);
    ...
    std::set<std::shared_ptr<CharacterFreeSlots>, SlotCompare> freeSlots{};
    ToolboxIni* freeSlotsIni{};
