    ...
    if (visible) {
        ...
        if (ImGui::Begin(Name(), GetVisiblePtr(), GetWinFlags())) {
            for (const auto tbuild : teambuilds) {
                ...
                if (ImGui::Button(tbuild->name.c_str(), ImVec2(ImGui::GetContentRegionAvail().x - ImGui::GetStyle().ItemInnerSpacing.x - 60.0f * ImGui::GetStyle().FontScaleMain, 0))) {
                ...
                if (ImGui::Button("Send", ImVec2(60.0f * ImGui::GetStyle().FontScaleMain, 0))) {
                    Send(tbuild);
                }
                ImGui::PopID();
            }
