...
void AccountInventoryWindow::OnMapLoadedDelayed()
{
    ...
    if (characterChanged) {
        ...
        if (availableChars != lastAvailableChars) {
            ...
            std::unordered_set<std::wstring> removedChars;
            for (auto it = inventory.begin(); it != inventory.end();) {
                if (!IsChestBag((*it)->bag_id) && (*it)->account == account) {
                    if (auto avail = availableChars.find((*it)->character); avail == availableChars.end()) {
                        removedChars.insert((*it)->character);
            ...
            for (auto const & iniID: removedChars) {
                DeleteFileW(iniByCharacter[iniID]->location_on_disk.wstring().c_str());
            }
        }
...
void AccountInventoryWindow::OnItemTooltip(std::shared_ptr<MergeStack> ms, std::wstring account)
{
    ...
    for (auto it = ms->i.begin(); it != ms->i.end(); it++) {
        ImGui::Text("%d", (*it)->itemPartial.item_id);
        int styleCount = 0;
...
void AccountInventoryWindow::Draw(IDirect3DDevice9*)
{
    ...
    if (visible) {
        ...
        if (ImGui::GetContentRegionAvail().y < 0 || !ImGui::BeginTable("###itemstable", ColumnID_Max, flags, ImVec2(innerWidth, itemstableHeight))) {
            ImGui::End();
            return;
        }
        ...
        if (grid_view) {
            ...
            ImGui::EndTable();
            if (ImGui::GetContentRegionAvail().y < 0 || !ImGui::BeginTable("###itemgrid", (int)(innerWidth / (3.3f*ImGui::GetTextLineHeight() + itemSpacing)), ImGuiTableFlags_NoBordersInBody | ImGuiTableFlags_ScrollY, ImVec2(innerWidth, 0.f))) {
                ImGui::End();
                return;
            }
            ImGui::TableNextRow();
...
void AccountInventoryWindow::LoadFromFiles(bool onlyForeign)
{
        ...
        for (const ToolboxIni::Entry& entry : entries) {
            ...
            if (ini->iniID.empty()) {
                if (IsChestBag(i->bag_id)) {
                    ...
                    ini->iniID = i->character;
                }
                iniByCharacter[ini->iniID] = ini;
            } else if (ini->iniID != i->account && ini->iniID != i->character) {
                continue;
            }
            if (onlyForeign && i->account == currentAccount) {
                // skip items from current account if called from LoadSettings or SaveToFiles
                ini->Reset();
                break;
            }
    ...
    for (auto it = iniByPath.begin(); it != iniByPath.end();) {
        if (visited.contains(it->first)) ++it;
        else {
            if (it->second->IsEmpty()) {
                DeleteFileW(it->second->location_on_disk.wstring().c_str());
            }
...
void AccountInventoryWindow::SaveToFiles()
{
    ...
    for (auto it = iniByCharacter.begin(); it != iniByCharacter.end(); ++it) {
        auto iniID = it->first;
        if (iniID != currentAccount) continue;
        if (auto avail = availableChars.find(iniID); avail == availableChars.end()) continue;
        if (!visited.contains(iniID)) {
            it->second->Reset();
        }
