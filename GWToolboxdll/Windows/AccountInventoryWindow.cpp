...
std::wstring AccountInventoryWindow::getIniID(std::shared_ptr<InventoryItem> i) {
            ...
            for (auto it = inventory.begin(); it != inventory.end();) {
                if (!IsChestBag((*it)->bag_id) && (*it)->account == currentAccount) {
                    if (auto avail = availableChars.find((*it)->character); avail == availableChars.end()) {
                        // not found
                        inventoryDirty.insert(getIniID(*it));
...
void AccountInventoryWindow::PreMapLoad()
{
    std::wstring currentAccount = GetAccountEmail();
    std::wstring currentCharacter = GW::AccountMgr::GetCurrentPlayerName();
    dbg += " preml " + TextUtils::WStringToString(currentAccount) + " " + TextUtils::WStringToString(currentCharacter);
...
{
    dbg += " mapl" + TextUtils::WStringToString(GW::AccountMgr::GetCurrentPlayerName());
    ...
    if (gwInventory) {
        ...
        for (uint32_t bag_id = 1; bag_id < std::size(gwInventory->bags); ++bag_id) {
            ...
            for (uint32_t slot = 0; slot < std::size(bag->items); ++slot) {
                ...
                if (item) {
                    ASSERT(inventoryLookup.contains(item->item_id));
                    ...
                    if (i->itemPartial.equipped != item->equipped) {
                        ...
                        inventoryDirty.insert(getIniID(i));
                    }
                } else {
...
void AccountInventoryWindow::OnItemTooltip(std::shared_ptr<MergeStack> ms)
{
    ...
    for (auto it = ms->i.begin(); it != ms->i.end(); it++) {
        ImGui::Text("%d %d", (*it)->itemPartial.item_id, freeSlots.size());
        int styleCount = 0;
...
void AccountInventoryWindow::Draw(IDirect3DDevice9*)
{
    ...
    // hide window between Loading and OnMapLoadedDelayed to avoid flickering
    //if (GW::Map::GetInstanceType() == GW::Constants::InstanceType::Loading || mapLoadedDelayedTrigger) return;
    if (visible) {
        ImGui::SetNextWindowCenter(ImGuiCond_FirstUseEver);
        ImGui::SetNextWindowSize(ImVec2(760.f * fontScale, 300.f * fontScale), ImGuiCond_FirstUseEver);
        if (!ImGui::Begin(Name(), GetVisiblePtr(), GetWinFlags()) || ImGui::IsWindowCollapsed()) {
            ImGui::End();
            return;
        }
        ImGui::Text("%s", dbg2.c_str());
        ImGui::InputText("###dbg", dbg.data(), dbg.size());
        std::wstring currentAccount = GetAccountEmail();
        ...
        if (ImGui::CollapsingHeader("Free Slots")) {
            if (!ImGui::BeginTable("###freeslots", 3, ImGuiTableFlags_BordersOuter | ImGuiTableFlags_BordersV | ImGuiTableFlags_NoBordersInBody | ImGuiTableFlags_SizingFixedFit)) {
                ImGui::End();
                return;
            }
            ImGui::TableSetupColumn("Character", ImGuiTableColumnFlags_WidthFixed);
            ImGui::TableSetupColumn("Equipment", ImGuiTableColumnFlags_WidthFixed);
            ImGui::TableSetupColumn("Inventory", ImGuiTableColumnFlags_WidthFixed);
            ...
            for (auto & freeSlot: freeSlots) {
                ...
                //if (free_equipment == 0 && free_inventory == 0) continue;
                ImGui::TableNextColumn();
                std::string suffix = "";
                if (freeSlot->account != currentAccount && freeSlot->character == L"(Chest)" && !freeSlot->account_representing_character.empty()) {
                ...
                ImGui::TableNextColumn();
                if (free_equipment) ImGui::Text("%d", free_equipment);
                ImGui::TableNextColumn();
                if (free_inventory) ImGui::Text("%d", free_inventory);
            }
            ImGui::EndTable();
        }
        ...
        if (!ImGui::BeginTable("###itemstable", ColumnID_Max, flags, ImVec2(innerWidth, itemstableHeight))) {
            ImGui::End();
            return;
        }
        ...
        if (grid_view) {
            ...
            ImGui::EndTable();
            if (!ImGui::BeginTable("###itemgrid", std::max(1, (int)(innerWidth / (3.3f*ImGui::GetTextLineHeight() + itemSpacing))), ImGuiTableFlags_NoBordersInBody | ImGuiTableFlags_ScrollY, ImVec2(innerWidth, 0.f))) {
                ImGui::End();
                return;
            }
            ImGui::TableNextRow();
...
{
    dbg += " lfile " + std::to_string(onlyForeign);
    ToolboxIni::TNamesDepend entries{};
    std::wstring currentAccount = GetAccountEmail();

    freeSlotsIni->LoadFile(Resources::GetSettingFile(FREE_SLOTS_INI_FILENAME).c_str());
    freeSlotsIni->GetAllSections(entries);
    std::set<std::shared_ptr<CharacterFreeSlots>, SlotCompare> freeSlotsRebuild{};
    for (const ToolboxIni::Entry& entry : entries) {
        const char* section = entry.pItem;
        auto freeSlot = std::make_shared<CharacterFreeSlots>();
        const char * account_b64 = freeSlotsIni->GetValue(section, "account", nullptr);
        if (account_b64) {
            freeSlot->account = TextUtils::Base64DecodeW(account_b64);
        }
        if (onlyForeign && freeSlot->account == currentAccount) continue;
        const char * character_b64 = freeSlotsIni->GetValue(section, "character", nullptr);
        if (character_b64) {
            freeSlot->character = TextUtils::Base64DecodeW(character_b64);
        }
        const char * account_representing_character_b64 = freeSlotsIni->GetValue(section, "account_character", nullptr);
        if (account_representing_character_b64) {
            freeSlot->account_representing_character = TextUtils::Base64DecodeW(account_representing_character_b64);
        }
        freeSlot->max_equipment = (int)(freeSlotsIni->GetLongValue(section, "maxequipment", 0));
        freeSlot->max_inventory = (int)(freeSlotsIni->GetLongValue(section, "maxinventory", 0));
        freeSlot->occupied_equipment = (int)(freeSlotsIni->GetLongValue(section, "occupiedequipment", 0));
        freeSlot->occupied_inventory = (int)(freeSlotsIni->GetLongValue(section, "occupiedinventory", 0));
    ...
    for (auto const &file: std::filesystem::directory_iterator{Resources::GetPath(L"inventories")}) {
        ...
        for (const ToolboxIni::Entry& entry : entries) {
            ...
            i->account = TextUtils::Base64DecodeW(ini->GetValue(section, "account", ""));
            i->character = TextUtils::Base64DecodeW(ini->GetValue(section, "character", ""));
            i->bag_id = (uint32_t)(ini->GetLongValue(section, "bagid", 1));
            if (ini->iniID.empty()) {
                ini->iniID = getIniID(i);
                ini->account = i->account;
                iniByCharacter[ini->iniID] = ini;
            } else if (ini->iniID != getIniID(i)) {
                continue;
            }
...
void AccountInventoryWindow::SaveToFiles()
{
    ...
    // separate loop because deleting from iniByCharacter in during the above loop get really unreadable
    ...
    freeSlotsIni->Reset();
    for (auto & freeSlot: freeSlots) {
        if (freeSlot->account.empty()) continue;
        if (freeSlot->character.empty()) continue;
        auto account_b64 = TextUtils::Base64EncodeW(freeSlot->account);
        auto character_b64 = TextUtils::Base64EncodeW(freeSlot->character);
        auto section = account_b64 + character_b64;
        freeSlotsIni->SetValue(section.c_str(), "account", account_b64.c_str());
        freeSlotsIni->SetValue(section.c_str(), "character", character_b64.c_str());
        if (!freeSlot->account_representing_character.empty()) {
            freeSlotsIni->SetValue(section.c_str(), "account_character", TextUtils::Base64EncodeW(freeSlot->account_representing_character).c_str());
        }
        freeSlotsIni->SetLongValue(section.c_str(), "maxequipment", freeSlot->max_equipment);
        freeSlotsIni->SetLongValue(section.c_str(), "maxinventory", freeSlot->max_inventory);
        freeSlotsIni->SetLongValue(section.c_str(), "occupiedequipment", freeSlot->occupied_equipment);
        freeSlotsIni->SetLongValue(section.c_str(), "occupiedinventory", freeSlot->occupied_inventory);
    }
    if (freeSlotsIni->SaveFile(Resources::GetSettingFile(FREE_SLOTS_INI_FILENAME).c_str()) != SI_OK) {
        Log::Error("Account Inventory: Failed to save free slot ini. Display of free inventory space will be wrong.");
    }
}

void AccountInventoryWindow::AddItem(uint32_t item_id)
{
    dbg += " a" + std::to_string(item_id);
    auto item = GW::Items::GetItemById(item_id);
    if (!(item && item->bag)) return;
    dbg += " " + std::to_string(item->model_id);
    ...
    if (auto it = freeSlots.find(std::make_shared<CharacterFreeSlots>(CharacterFreeSlots{currentAccount, i->character})); it != freeSlots.end()) {
        ...
        dbg += " fsmiss " + TextUtils::WStringToString(i->character);
    }
    if (auto it = inventory.find(i); it != inventory.end()) {
        ...
        if (inventoryLookup[oitem_id] == (*it)) {
...
void AccountInventoryWindow::ClearMissingItem(std::wstring account, std::wstring character, uint32_t hero_id, uint32_t bag_id, uint32_t slot)
{
    ...
    i->hero_id = hero_id;
    if (auto it = inventory.find(i); it != inventory.end()) {
        auto item_id = (*it)->itemPartial.item_id;
        dbg += " c" + std::to_string(item_id) + " " + std::to_string(item_id);
        dbg += " " + (inventoryLookup.contains(item_id) ? std::to_string(inventoryLookup[item_id]) : "False");
        // found
        //inventoryDirty.insert(getIniID(*it));
        //if (inventoryLookup.contains(item_id)) {
        //    RemoveItem(item_id);
        //} else {
