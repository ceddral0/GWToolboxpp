...
constexpr const wchar_t* INI_FILENAME = L"accountinventory.ini";
...
static bool checkIniDirty(FILETIME * last_ini_change)
{
    FILETIME ini_change;
    HANDLE f = CreateFileW(Resources::GetSettingFile(INI_FILENAME).c_str(),
        GENERIC_READ,
    ...
    bool res = GetFileTime(f, NULL, NULL, &ini_change);
    CloseHandle(f);
    if (!res) return false;
    if (CompareFileTime(&ini_change, last_ini_change) != 0) {
        *last_ini_change = ini_change;
        return true;
    }
    return false;
}
...
void AccountInventoryWindow::OnInventoryItemClicked(std::shared_ptr<InventoryItem> i) {
...
void AccountInventoryWindow::Initialize()
{
    ...
    for (auto message_id : ui_messages) {
        GW::UI::RegisterUIMessageCallback(&OnUIMessage_HookEntry, message_id,
...
{
    if (last_track_inventory && !track_inventory) {
        inventory.clear();
        inventoryLookup.clear();
        inventorySorted.clear();
        SaveToFile();
    }
    last_track_inventory = track_inventory;
    {
...
void AccountInventoryWindow::OnMapLoadedDelayed()
{
    ...
    std::wstring character = GW::AccountMgr::GetCurrentPlayerName();
    if (lastCharacter != character) {
        lastCharacter = character;
        characterChanged = true;
    }
    std::wstring account = GetAccountEmail();
    ...
    if (gwInventory) {
        for (uint32_t bag_id = 1; bag_id < std::size(gwInventory->bags); ++bag_id) {
            ...
            if (!bag) continue; // skip absent
            for (uint32_t slot = 0; slot < std::size(bag->items); ++slot) {
                auto item = bag->items[slot];
                if (item && inventoryLookup.contains(item->item_id)) {
                ...
                if (!item && characterChanged) {
                    // on character change clear empty inventory slots, in case inventory was modified without us tracking it
        ...
        needsSorting = true; // show updated equipped flags
        inventoryDirty = true;
    }
    if (characterChanged) {
        ...
        if (availableChars != lastAvailableChars) {
            ...
            for (auto it = inventory.begin(); it != inventory.end();) {
                if (!IsChestBag((*it)->bag_id) && (*it)->account == account) {
    ...
    if (save_often && GW::Map::GetInstanceType() == GW::Constants::InstanceType::Outpost) {
        SaveToFile(); // save inventory in outposts only to avoid impacting gameplay
    }
...
void AccountInventoryWindow::OnMapLoaded()
{
...
void AccountInventoryWindow::OnItemTooltip(std::shared_ptr<MergeStack> ms, std::wstring account)
{
    ...
    for (auto it = ms->i.begin(); it != ms->i.end(); it++) {
        ...
        // For items in chest it could be helpful to show which account they are on,
        // but I'd rather not have streaming players accidentaly reveal their email adress by showing it here.
        bool reprint = (*it)->character != prevCharacter;
        if (reprint) {
            ImGui::Text("%s", TextUtils::WStringToString((*it)->character).c_str());
        }
...
void AccountInventoryWindow::SortInventory(ImGuiTableSortSpecs* sortSpecs, std::wstring account)
{
    ...
    for (auto it = inventory.begin(); it != inventory.end(); ++it) {
        ...
        if (merge_stacks && mergedStacks.contains(merge_id)) {
            ...
            ms = std::make_shared<MergeStack>(account);
    ...
    if (inventorySorted.size() > 1) {
        std::sort(inventorySorted.begin(), inventorySorted.end(), ItemCompare{sortSpecs, account});
    }
...
void AccountInventoryWindow::Draw(IDirect3DDevice9*)
{
    ...
    if (visible) {
        ImGui::SetNextWindowCenter(ImGuiCond_FirstUseEver);
        ImGui::SetNextWindowSize(ImVec2(885.f * fontScale, 300.f * fontScale), ImGuiCond_FirstUseEver);
        // supposedly Begin returns false if the window is collapsed, but if the window is visible and collapsed on loading in,
        // BeginTable with ImGuiTableFlags_ScrollY further down causes a crash. Checking IsWindowCollapsed does prevent the crash
        if (ImGui::Begin(Name(), GetVisiblePtr(), GetWinFlags()) && !ImGui::IsWindowCollapsed()) {
            ...
            ImGui::SameLine(ImGui::GetContentRegionAvail().x - itemSpacing - 100.f * fontScale);
            ImGui::SetNextItemWidth(100.f);
            if (ImGui::Button("Load Inventories")) {
                if (!track_inventory) {
                    ImGui::ConfirmDialog("Inventory tracking is currently disabled.\n\nEnable inventory tracking?", [&track_inventory = track_inventory](bool result, void*){if (result) track_inventory = true;});
                } else {
            ...
            ImGui::BeginTable("###itemstable", ColumnID_Max, flags, ImVec2(innerWidth, itemstableHeight));
            ImGui::TableSetupColumn("Character", ImGuiTableColumnFlags_DefaultSort | ImGuiTableColumnFlags_WidthFixed, 0.f, ColumnID_Character);
            ImGui::TableSetupColumn("Location", ImGuiTableColumnFlags_WidthFixed, 0.f, ColumnID_Location);
            ImGui::TableSetupColumn("Model ID", ImGuiTableColumnFlags_WidthFixed, 0.f, ColumnID_ModelID);
            ImGui::TableSetupColumn("Item", ImGuiTableColumnFlags_WidthFixed, 0.f, ColumnID_Description);
            ...
            if (!track_inventory) {
                ImGui::Checkbox("Track Inventory", &track_inventory);
            }

            ImGuiTableSortSpecs* sortSpecs = ImGui::TableGetSortSpecs();
            if (needsSorting || (sortSpecs && sortSpecs->SpecsDirty)) {
                SortInventory(sortSpecs, account);
            }
            ...
            if (grid_view) {
                ...
                ImGui::EndTable();
                ImGui::BeginTable("###itemgrid", (int)(innerWidth / (3.3f*ImGui::GetTextLineHeight() + itemSpacing)), ImGuiTableFlags_NoBordersInBody | ImGuiTableFlags_ScrollY, ImVec2(innerWidth, 0.f));
                ImGui::TableNextRow();
                ImGui::TableNextColumn();
            }
            ...
            for (auto & ims: inventorySorted) {
                ...
                if (grid_view) {
                    ...
                    if (ImGui::IsItemHovered()) {
                        ImGui::SetTooltip([ims, account]() { AccountInventoryWindow::Instance().OnItemTooltip(ims, account); });
                    }
                    ImGui::TableNextColumn();
                } else { // detail view
...
void AccountInventoryWindow::DrawSettingsInternal()
{
    ...
    ImGui::Checkbox("###account_inventory_track_inventory", &track_inventory);
    ImGui::SameLine();
    ...
    ImGui::Checkbox("###account_inventory_save_often", &save_often);
    ImGui::SameLine();
    ImGui::Text("Save inventory data when loading into an outpost. Noticably increases outpost loading time, but reduces data loss on crashes.");
    ImGui::PopTextWrapPos();
}
...
std::string AccountInventoryWindow::KeyToString(std::shared_ptr<InventoryItem> i) const
{
    char buf[9];
    std::string out;
    out.append(TextUtils::Base64EncodeW(i->account));
    out.append(TextUtils::Base64EncodeW(i->character));
...
void AccountInventoryWindow::LoadFromFile(bool onlyForeign)
{
    if (!checkIniDirty(&last_ini_change)) return;
    inifile->Reset();
    std::wstring account = GetAccountEmail();
    if (onlyForeign) {
        for (auto it = inventory.begin(); it != inventory.end();) {
            if ((*it)->account != account) it = inventory.erase(it);
            else ++it;
        }
    } else {
        inventory.clear();
    ...
    if (inifile->LoadFile(Resources::GetSettingFile(INI_FILENAME).c_str())) return;
    ...
    for (const ToolboxIni::Entry& entry : entries) {
        ...
        auto i = std::make_shared<InventoryItem>();
        i->account = TextUtils::Base64DecodeW(inifile->GetValue(section, "account", ""));
        i->character = TextUtils::Base64DecodeW(inifile->GetValue(section, "character", ""));
        if (onlyForeign && i->account == account) continue; // skip items from current account if called from LoadSettings or SaveToFile
        i->hero_id = (uint32_t)(inifile->GetLongValue(section, "heroid", 0));
        i->bag_id = (uint32_t)(inifile->GetLongValue(section, "bagid", 1));
        i->slot = (uint32_t)(inifile->GetLongValue(section, "slot", 0));

        const char * info_string_b64 = inifile->GetValue(section, "info", nullptr);
        if (info_string_b64) {
            i->info_string = TextUtils::Base64DecodeW(info_string_b64);
            i->itemPartial.info_string = i->info_string.data();
        }
        const char * single_item_name_b64 = inifile->GetValue(section, "singleitemname", nullptr);
        ...
        const char * complete_name_enc_b64 = inifile->GetValue(section, "completename", nullptr);
        ...
        const char * name_enc_b64 = inifile->GetValue(section, "name", nullptr);
        if (name_enc_b64) {
            i->name_enc = TextUtils::Base64DecodeW(name_enc_b64);
            i->itemPartial.name_enc = i->name_enc.data();
        }
        const char * customized_b64 = inifile->GetValue(section, "customized", nullptr);
        if (customized_b64) {
            i->customized = TextUtils::Base64DecodeW(customized_b64);
            i->itemPartial.customized = i->customized.data();
        }
        i->itemPartial.type = (GW::Constants::ItemType)(inifile->GetLongValue(section, "type", 0));
        i->itemPartial.model_id = (uint32_t)(inifile->GetLongValue(section, "modelid", 0));
        i->itemPartial.model_file_id = (uint32_t)(inifile->GetLongValue(section, "modelfileid", 0));
        i->itemPartial.interaction = (uint32_t)(inifile->GetLongValue(section, "interaction", 0));
        i->itemPartial.quantity = (uint16_t)(inifile->GetLongValue(section, "quantity", 0));
        i->itemPartial.equipped = (uint8_t)(inifile->GetLongValue(section, "equipped", 0));
...
// SaveToFile can take a long time (seconds) for large inventories. Be careful where to call it
void AccountInventoryWindow::SaveToFile()
{
    ...
    for (auto it = inventory.begin(); it != inventory.end(); ++it) {
        auto i = *it;
        if (account.empty() || i->account != account) continue; // skip foreign items, only update what belongs to us
        auto section = KeyToString(i);
        inifile->SetValue(section.c_str(), "account", TextUtils::Base64EncodeW(i->account).c_str());
        inifile->SetValue(section.c_str(), "character", TextUtils::Base64EncodeW(i->character).c_str());
        inifile->SetLongValue(section.c_str(), "heroid", (long)i->hero_id);
        inifile->SetLongValue(section.c_str(), "bagid", (long)i->bag_id);
        inifile->SetLongValue(section.c_str(), "slot", (long)i->slot);
        if (i->itemPartial.info_string) {
            inifile->SetValue(section.c_str(), "info", TextUtils::Base64EncodeW(i->itemPartial.info_string).c_str());
        }
        if (i->itemPartial.single_item_name) {
            inifile->SetValue(section.c_str(), "singleitemname", TextUtils::Base64EncodeW(i->itemPartial.single_item_name).c_str());
        }
        if (i->itemPartial.complete_name_enc) {
            inifile->SetValue(section.c_str(), "completename", TextUtils::Base64EncodeW(i->itemPartial.complete_name_enc).c_str());
        }
        if (i->itemPartial.name_enc) {
            inifile->SetValue(section.c_str(), "name", TextUtils::Base64EncodeW(i->itemPartial.name_enc).c_str());
        }
        if (i->itemPartial.customized) {
            inifile->SetValue(section.c_str(), "customized", TextUtils::Base64EncodeW(i->itemPartial.customized).c_str());
        }
        inifile->SetLongValue(section.c_str(), "type", (long)i->itemPartial.type);
        inifile->SetLongValue(section.c_str(), "modelid", (long)i->itemPartial.model_id);
        inifile->SetLongValue(section.c_str(), "modelfileid", (long)i->itemPartial.model_file_id);
        inifile->SetLongValue(section.c_str(), "interaction", (long)i->itemPartial.interaction);
        inifile->SetLongValue(section.c_str(), "quantity", (long)i->itemPartial.quantity);
        inifile->SetLongValue(section.c_str(), "equipped", (long)i->itemPartial.equipped);
    }
...
void AccountInventoryWindow::AddItem(uint32_t item_id)
{
    ...
    if (i->bag_id == (uint32_t)GW::Constants::Bag::Equipped_Items && (GW::Inventory *)item->bag->bag_array != GW::Items::GetInventory()) {
        ...
        if (item->slot == 6) {
            ...
            for (auto oItem: item->bag->items) {
                if (!oItem) {
                    auto oi = std::make_shared<InventoryItem>();
                    oi->account = account;
                    oi->character = i->character;
                    oi->bag_id = i->bag_id;
                    oi->slot = slot;
                    oi->hero_id = i->hero_id;
                    if (auto it = inventory.find(oi); it != inventory.end()) {
                        RemoveItem((*it)->itemPartial.item_id);
                        // Most likely the missing item was still in our inifile but removed ingame.
                        // In this case we won't know an item_id, but still want to remove it from inventory
                        inventory.erase(it);
...
{
    if (!track_inventory || !inventoryLookup.contains(item_id)) return;
